...
public class DataSourceInternalWriterHelper {
  ...
  public void commit(List<WriteStatus> writeStatuses) {
    try {
      writeClient.commitStats(instantTime, hoodieTable.getContext().parallelize(writeStatuses), writeStatuses.stream().map(WriteStatus::getStat).collect(Collectors.toList()), Option.of(extraMetadata),
