...
public class ValidateDatasetNode extends DagNode<Boolean> {
  ...
  public void execute(ExecutionContext context, int curItrCount) throws Exception {
    ...
    if (config.getIterationCountToExecute() == curItrCount) {
      ...
      List<Row> hudiDfRows = hudiDf.collectAsList();
      log.warn("Total read time for all of hudi data (spark ds read) : " + (System.currentTimeMillis() - startTime));
      ...
      if (config.isValidateHive()) {
        ...
        if (!config.readPartialData()) {
          long startTimeMs = System.currentTimeMillis();
          Dataset<Row> cowDf = session.sql("SELECT * FROM " + database + "." + tableName);
          List<Row> cowDfList = cowDf.collectAsList();
          log.warn("Hive sql : Reading full data : " + (System.currentTimeMillis() - startTimeMs));
          ...
          long startTimeMs = System.currentTimeMillis();
          Dataset<Row> cowDf = session.sql("SELECT * FROM " + database + "." + tableName + " where fare > " + config.getMinFareValueToRead());
          List<Row> cowDfList = cowDf.collectAsList();
          log.warn("Hive sql : Reading partial data : " + (System.currentTimeMillis() - startTimeMs));
        }
      }
