...
public class ValidateDatasetNode extends DagNode<Boolean> {
  ...
  public void execute(ExecutionContext context) throws Exception {
    ...
    /*for (FileStatus fileStatus : fileStatuses) {
      log.warn("Listing all Micro batches to be validated :: " + fileStatus.getPath().toString());
    }*/
    ...
    Dataset<Row> inputSnapshotDf = inputDf.groupByKey(
        ...
        .map((MapFunction<Tuple2<String, Row>, Row>) value -> value._2, encoder).filter("_hoodie_is_deleted == false");

    //.filter((FilterFunction<Row>) o -> !((Boolean) o.getAs("_hoodie_is_deleted")));
    ...
    if (inputSnapshotDf.except(intersectionDf).count() != 0) {
      ...
      log.warn("Intersected df ::::: " + intersectionDf.count());
      List<Row> intersectedRows = intersectionDf.takeAsList(50);
      for (Row row : intersectedRows) {
        log.warn("intersected Row " + row.mkString(","));
      }

      log.warn("Input Df ::" + inputSnapshotDf.count());
      List<Row> inputRows = inputSnapshotDf.takeAsList(50);
      for (Row row : inputRows) {
        log.warn("Input Row " + row.mkString(","));
      }
      log.error("Hudi Df :: " + trimmedDf.count());
      inputRows = trimmedDf.takeAsList(50);
      for (Row row : inputRows) {
        log.warn("Hudi Row " + row.mkString(","));
      }
    ...
    if (config.isDeleteInputData()) {
      log.warn("ValidateDatasetNode: Deleting input data ----------- ");
