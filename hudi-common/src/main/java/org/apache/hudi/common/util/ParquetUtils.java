...
public class ParquetUtils extends BaseFileUtils {
   ...
   */
  public Collection<HoodieColumnStatsMetadata<Comparable>> readColumnStatsFromParquetMetadata(Configuration conf, String partitionPath, Path parquetFilePath) {

      ParquetMetadata metadata = readMetadata(conf, parquetFilePath);
      // collect stats from all parquet blocks
      Map<String, List<HoodieColumnStatsMetadata<Comparable>>> columnToStatsListMap = metadata.getBlocks().stream().flatMap(blockMetaData -> {
        return blockMetaData.getColumns().stream().map(columnChunkMetaData ->
            new HoodieColumnStatsMetadata<>(partitionPath, parquetFilePath.getName(), columnChunkMetaData.getPath().toDotString(),
      ...
      // we only intend to keep file level statistics (not per-block level statisitcs in index. In future, we can change to store row group level stats
  ...
  private HoodieColumnStatsMetadata<Comparable> combineRanges(HoodieColumnStatsMetadata<Comparable> range1,
    ...
    return new HoodieColumnStatsMetadata<>(range1.getPartitionPath(), range1.getFilePath(), range1.getColumnName(), minValue, maxValue, false);
  }
