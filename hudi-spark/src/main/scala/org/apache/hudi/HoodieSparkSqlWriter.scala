...
private[hudi] object HoodieSparkSqlWriter {
  ...
  def write(sqlContext: SQLContext,
    ...
    val compressionCodecStr = parameters("hoodie.bulkinsert.compression.codec")
    var compressionCodec = "UNCOMPRESSED"
    if (!compressionCodecStr.isEmpty) {
      compressionCodec = compressionCodecStr
    }

    println("Parallelism :: " + parallelism + ", codec " + compressionCodec + ", operation " + operation)
    ...
    if (operation.equalsIgnoreCase("bulk_insert_direct_parquet_write_support")) {
      ...
      val writtenRows: Dataset[java.lang.Boolean] = WriteHelper.writeToParquet(df, basePath.toString, RowEncoder(df.schema), serConfig,
        parallelism, false, compressionCodec)
      writtenRows.collect()
      (true, org.apache.hudi.common.util.Option.of("Completed"))
    } else if (operation.equalsIgnoreCase("bulk_insert_direct_avro_write_support")) {
      val writtenRows: Dataset[java.lang.Boolean] = WriteHelper.writeToParquet(df, basePath.toString, RowEncoder(df.schema), serConfig,
        parallelism, true, compressionCodec)
      writtenRows.collect()
      (true, org.apache.hudi.common.util.Option.of("Completed"))
    }  else {
      ...
      val (writeStatuses, writeClient: HoodieWriteClient[HoodieRecordPayload[Nothing]]) =
        ...
        } else if (!operation.equalsIgnoreCase(DELETE_OPERATION_OPT_VAL)) {
          // register classes & schemas
          val structName = s"${tblName.get}_record"
          val nameSpace = s"hoodie.${tblName.get}"
          sparkContext.getConf.registerKryoClasses(
