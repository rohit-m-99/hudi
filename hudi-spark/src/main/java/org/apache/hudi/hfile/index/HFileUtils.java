...
public class HFileUtils implements Serializable {
  ...
  public static void writeToHFile(Configuration conf, Path hFilePath, List<String> keysToWrite, int minBlockSze) throws IOException {
    try {
   ...
   */
    ...
    /*  System.out.println("ALL: Total time to look up " + keysToLookUp.size() + " keys. time to read all records and look up : "
          + (System.currentTimeMillis() - startTime) + ", time to look up " + (System.currentTimeMillis() - startTime1));
  ...
  public static Map<String, String> readAllRecordsWithSeek(Path hFilePath, Configuration conf, List<byte[]> keysToLookUp) throws IOException {
    ...
    //System.out.println("SEEK: Total time to look up " + keysToLookUp.size() + " keys " + (System.currentTimeMillis() - startTime));
