...
public class TestHoodieSparkMergeOnReadTableCompaction extends SparkClientFunctionalTestHarness {
  ...
  public void testWriteDuringCompaction(String payloadClass) throws IOException {
    ...
    client.commitStats(insertTime, HoodieJavaRDD.of(jsc().parallelize(writeStatuses)), writeStatuses.stream().map(WriteStatus::getStat).collect(Collectors.toList()), Option.empty(),
  ...
  private List<WriteStatus> writeData(String instant, int numRecords, boolean doCommit) {
    ...
    if (doCommit) {
      ...
      boolean committed = client.commitStats(instant, HoodieJavaRDD.of(jsc().parallelize(writeStatuses)), writeStats, Option.empty(), metaClient.getCommitActionType());
