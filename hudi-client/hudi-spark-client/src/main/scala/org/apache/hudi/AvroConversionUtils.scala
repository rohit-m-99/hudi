...
import org.apache.hudi.common.model.{HoodieKey, HoodieRowRecord}
...
object AvroConversionUtils {
  ...
  /*
  def createDataset(df: DataFrame, avroSchema: Schema, structName: String, recordNamespace: String)
  : Dataset[HoodieRowRecord] = {
    // Use the Avro schema to derive the StructType which has the correct nullability information
    val dataType = SchemaConverters.toSqlType(avroSchema).dataType.asInstanceOf[StructType]
    val encoder = RowEncoder.apply(dataType).resolveAndBind()
    df.mapPartitions( {
      records =>
        if(records.isEmpty) Iterator.empty
          ...
          records.map { x =>
            {
              val genRec = convertor(x).asInstanceOf[GenericRecord]
              new HoodieRowRecord(HoodieAvroUtils.avroToBytes(genRec))
