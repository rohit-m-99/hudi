 ...
 */
public class GenericRecordPayloadSizeEstimator implements Serializable {
  ...
  public GenericRecordPayloadSizeEstimator(Schema schema) {
  ...
  private long typeEstimate(Schema schema) {
    ...
    switch (localSchema.getType()) {
      ...
      case ARRAY:
        if (GenericRecordPayloadGenerator.isPrimitive(localSchema.getElementType())) {
          counter.addAndGet(1);
        }
        Schema elementSchema = localSchema.getElementType();
        return typeEstimate(elementSchema);
      case MAP:
        if (GenericRecordPayloadGenerator.isPrimitive(localSchema.getValueType())) {
          counter.addAndGet(1);
        }
