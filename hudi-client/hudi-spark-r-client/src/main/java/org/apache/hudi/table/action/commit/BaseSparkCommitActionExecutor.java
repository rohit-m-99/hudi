...
public abstract class BaseSparkCommitActionExecutor<Row> extends
    BaseCommitActionExecutor<Row, Dataset<Row>, Dataset<Row>, Dataset<HoodieInternalWriteStatus>, HoodieWriteMetadata> {
  ...
  @Override
  public HoodieWriteMetadata<Dataset<HoodieInternalWriteStatus>> execute(Dataset<Row> inputRecordsRDD) {
    HoodieWriteMetadata<Dataset<HoodieInternalWriteStatus>> result = new HoodieWriteMetadata<>();
    // Cache the tagged records, so we don't end up computing both
    // TODO: Consistent contract in HoodieWriteClient regarding preppedRecord storage level handling

    if (inputRecordsRDD.storageLevel() == StorageLevel.NONE()) {
      inputRecordsRDD.persist(StorageLevel.MEMORY_AND_DISK_SER());
    } else {
      LOG.info("RDD PreppedRecords was persisted at: " + inputRecordsRDD.storageLevel());
    }
    ...
    Dataset<Row> partitionedRecords = partition(inputRecordsRDD, partitioner);
    Dataset<HoodieInternalWriteStatus> writeStatusRDD = partitionedRecords.mapPartitionsWithIndex((partition, recordItr) -> {
  ...
  private Pair<HashMap<String, WorkloadStat>, WorkloadStat> buildProfile(Dataset<Row> inputRecordsRDD) {
  ...
  private Dataset<Row> partition(Dataset<Row> dedupedRecords, Partitioner partitioner) {
    JavaPairRDD<Tuple2, Row> mappedRDD = dedupedRecords.mapToPair(
        record -> new Tuple2<>(new Tuple2<>(record.getKey(), Option.ofNullable(record.getCurrentLocation())), record));

    JavaPairRDD<Tuple2, Dataset<Row>> partitionedRDD;
  ...
  protected void updateIndexAndCommitIfNeeded(Dataset<HoodieInternalWriteStatus> writeStatusRDD, HoodieWriteMetadata result) {
    ...
    Dataset<HoodieInternalWriteStatus> statuses = table.getIndex().updateLocation(writeStatusRDD, context, table);
  ...
  @Override
  protected void commit(Option<Map<String, String>> extraMetadata, HoodieWriteMetadata<Dataset<HoodieInternalWriteStatus>> result) {
  ...
  protected HoodieMergeHandle getUpdateHandle(String partitionPath, String fileId, Iterator<Row> recordItr) {
    if (table.requireSortedRecords()) {
  ...
  protected HoodieMergeHandle getUpdateHandle(String partitionPath, String fileId,
                                              Map<String, Row> keyToNewRecords,
  ...
  @Override
  public Iterator<List<WriteStatus>> handleInsert(String idPfx, Iterator<Row> recordItr)
      throws Exception {
    // This is needed since sometimes some buckets are never picked in getPartition() and end up with 0 records
    if (!recordItr.hasNext()) {
