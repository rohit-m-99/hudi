...
public class BulkInsertCommitActionExecutorRows<T extends HoodieRecordPayload<T>>
    extends CommitActionExecutor<T> {

  private final Dataset<Row> inputRecordsRDD;
  private final Option<UserDefinedBulkInsertPartitioner> bulkInsertPartitioner;

  public BulkInsertCommitActionExecutorRows(JavaSparkContext jsc,
                                            HoodieWriteConfig config, HoodieTable table,
                                            String instantTime, Dataset<Row> inputRecordsRDD,
  ...
  public HoodieWriteMetadata execute() {
    try {
      return BulkInsertHelperRows.bulkInsertRows(inputRecordsRDD, instantTime, (HoodieTable<T>) table, config,
